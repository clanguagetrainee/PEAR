...
def resolve_api(fqn: str, version: str, api_type: str, kb: KnowledgeBase,
    ...
    追 value、定义则识别转发/嵌套调用追被调 API，沿 import 链递归到真实定义或
    库外（解析失败即空推荐，不做同名近似兜底）。
    ...
    返回值：
        ResolvedApi：kind 语义见 model.ResolvedApi；original_fqn 恒为最外层输入；
            ...
            nested_external / unknown 时向最外层透传）。
    """
    return ImportResolver(provider).resolve(fqn, version, api_type, visited)
