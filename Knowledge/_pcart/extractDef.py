...
class Def2format:
    ...
    #  @param source_root The version source root (git worktree checkout). Used
    #         to build relativePath, whose directory depth must match the
    #         absolute path, otherwise shortenPath stops ascending too early.
    #  @param pkg_root The lib package root dir. Used to build the module prefix.
    #  @param libName The lib package name
    #  @return None
    def toFormat(self,filePath,source_root,pkg_root,libName):
        # vendored 修改点：原版按 site-packages 路径切分构造 prefix 与 relativePath。
        # 我们的源码来自 git worktree，故拆成两个根：
        #   relativePath 相对源码根 source_root（含完整包层级——shortenPath 依赖该
        #     层级数逐级上溯 __init__.py，若少一层会在包根下一层提前终止，漏掉公开名
        #     缩短，如 django.get_version 丢失的根因）；
        #   prefix 相对包目录 pkg_root，若首段非库包名则补上库名前缀。
        self._relativePath=os.path.relpath(filePath,source_root)
        s=os.path.relpath(filePath,pkg_root)
        s=s.replace('\\','.').replace('/','.')
        pos=s.rfind('.')
        s=s[0:pos]
        if not (s==libName or s.startswith(libName+'.')):
            s=libName+'.'+s
