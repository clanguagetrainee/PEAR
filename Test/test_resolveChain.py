...
#  method self 调用、库外判定（未 import 的名字）。用真实 fake_repo + SourceProvider，
#  不碰手工 kb / FakeProvider。
...
def test_cross_module_import_forward(fake_kb, provider):
    ...
    r = resolve_api(src, VERSION, 'function', fake_kb, provider)
...
def test_method_self_call(fake_kb, provider):
    """method 内 self._impl_method(x) -> 解析到同类方法（精确，非近似）。"""
    method = f'{MOD}.CrossClass.run'
    impl = f'{MOD}.CrossClass._impl_method'
    r = resolve_api(method, VERSION, 'method', fake_kb, provider)
    assert r.kind == 'forward'
    assert r.resolved_fqn == impl
    assert r.definition is not None and 'def _impl_method' in r.definition


def test_external_call_unresolved(fake_kb, provider):
    """库外调用 np.where(x)（np 未 import）-> nested_external 空推荐。"""
